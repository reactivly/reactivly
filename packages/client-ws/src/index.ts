export class EndpointsWSClient {
  ...
  subscribe<K extends string>(endpoint: K, params: any, cb: (data: any) => void) {
    ...
    if (!set) {
      ...
      if (this.ws!.readyState === WebSocket.OPEN) this.ws!.send(subscribeMsg);
      else this.ws!.addEventListener("open", () => this.ws!.send(subscribeMsg), { once: true });
    }
    set.add(cb);
    return () => {
      set!.delete(cb);
      if (set!.size === 0) {
        const unsubscribeMsg = JSON.stringify({ type: "unsubscribe", endpoint, params });
        if (this.ws?.readyState === WebSocket.OPEN) this.ws.send(unsubscribeMsg);
      }
    };
  }
  ...
  call<K extends string>(endpoint: K, params: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      else this.ws.addEventListener("open", () => this.ws!.send(msg), { once: true });
      setTimeout(() => {
